...
buffer_flip_vertical(struct lab_data_buffer *src)
{
	if (!src || !src->surface) {
		return NULL;
	}
	struct lab_data_buffer *dst = buffer_create_cairo(src->logical_width,
		src->logical_height, 1.0f);
	if (!dst || !dst->surface) {
		return NULL;
	}
	cairo_t *cairo = cairo_create(dst->surface);
	cairo_translate(cairo, 0, src->logical_height);
...
static const float mac_close_button_color[4] = { 1.0f, 0.372f, 0.341f, 1.0f };
static const float mac_minimize_button_color[4] = { 0.996f, 0.737f, 0.180f, 1.0f };
static const float mac_maximize_button_color[4] = { 0.157f, 0.784f, 0.251f, 1.0f };
static const float mac_ontop_button_color[4] = { 0.557f, 0.557f, 0.576f, 1.0f };
static const float mac_ontop_toggled_color[4] = { 0.62f, 0.40f, 0.91f, 1.0f };

static void
draw_mac_button_circle(cairo_t *cairo, int w, int h, const float color[4])
{
	double radius = MIN(w, h) / 2.0 - 2.0;
	double cx = w / 2.0;
	double cy = h / 2.0;

	cairo_new_sub_path(cairo);
	cairo_arc(cairo, cx, cy, MAX(radius, 1.0), 0, 360 * deg);
	set_cairo_color(cairo, color);
	cairo_fill(cairo);
}

static void
draw_macos_close_circle(cairo_t *cairo, int w, int h)
{
	draw_mac_button_circle(cairo, w, h, mac_close_button_color);
}

static void
draw_macos_minimize_circle(cairo_t *cairo, int w, int h)
{
	draw_mac_button_circle(cairo, w, h, mac_minimize_button_color);
}

static void
draw_macos_maximize_circle(cairo_t *cairo, int w, int h)
{
	draw_mac_button_circle(cairo, w, h, mac_maximize_button_color);
}

static void
draw_macos_ontop_circle(cairo_t *cairo, int w, int h)
{
	draw_mac_button_circle(cairo, w, h, mac_ontop_button_color);
}

static void
draw_macos_ontop_toggled_circle(cairo_t *cairo, int w, int h)
{
	draw_mac_button_circle(cairo, w, h, mac_ontop_toggled_color);
}

static struct lab_img *
create_macos_button_circle(lab_img_modifier_func_t modifier)
{
	static const char empty_bitmap[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	float transparent[4] = { 0, 0, 0, 0 };
	struct lab_img *img = lab_img_load_from_bitmap(empty_bitmap, transparent);
	if (img) {
		lab_img_add_modifier(img, modifier);
	}
	return img;
}
...
load_button(struct theme *theme, struct button *b, enum ssd_active_state active)
{
	...
	if (b->type == LAB_NODE_BUTTON_CLOSE
			|| b->type == LAB_NODE_BUTTON_ICONIFY
			|| b->type == LAB_NODE_BUTTON_MAXIMIZE
			|| b->type == LAB_NODE_BUTTON_ALWAYS_ON_TOP) {
		lab_img_modifier_func_t modifier = NULL;

		switch (b->type) {
		case LAB_NODE_BUTTON_CLOSE:
			modifier = draw_macos_close_circle;
			break;
		case LAB_NODE_BUTTON_ICONIFY:
			modifier = draw_macos_minimize_circle;
			break;
		case LAB_NODE_BUTTON_MAXIMIZE:
			modifier = draw_macos_maximize_circle;
			break;
		case LAB_NODE_BUTTON_ALWAYS_ON_TOP:
			modifier = (b->state_set & LAB_BS_TOGGLED)
				? draw_macos_ontop_toggled_circle
				: draw_macos_ontop_circle;
			break;
		default:
			break;
		}
		...
		if (modifier) {
			*img = create_macos_button_circle(modifier);
		}
	}
...
create_corners(struct theme *theme)
{
	...
	const float frame_black[4] = {0, 0, 0, 0.62f};
	float frame_border[4] = {0, 0, 0, 0.62f};
	cairo_pattern_t *frame_pattern = color_to_pattern(frame_black);
	...
	FOR_EACH_ACTIVE_STATE(active) {
		...
		struct wlr_box bottom_box = box;
		bottom_box.height = corner_width + theme->border_width;
		ctx.box = &bottom_box;
		ctx.corner = ROUNDED_CORNER_TOP_LEFT;
		struct lab_data_buffer *tmp_bottom_left = rounded_rect(&ctx);
		ctx.corner = ROUNDED_CORNER_TOP_RIGHT;
		struct lab_data_buffer *tmp_bottom_right = rounded_rect(&ctx);
		theme->window[active].corner_bottom_left_normal =
			buffer_flip_vertical(tmp_bottom_left);
		theme->window[active].corner_bottom_right_normal =
			buffer_flip_vertical(tmp_bottom_right);
		zdrop(&tmp_bottom_left);
		zdrop(&tmp_bottom_right);
	}
	zfree_pattern(frame_pattern);
}
