...
static void apply_window_rounded_corners(struct ssd *ssd);
...
ssd_titlebar_create(struct ssd *ssd)
{
	...
	apply_window_rounded_corners(ssd);
}
...
static void
apply_window_rounded_corners(struct ssd *ssd)
{
    struct view *view = ssd->view;

    if (!view->scene_tree || !view->surface) {
        return;
    }
    ...
    /* Get corner radius, default to 8px */
    int corner_radius = rc.corner_radius > 0 ? rc.corner_radius : 8;
    ...
    for (int y = 0; y < corner_radius; y++) {
        int x_start = corner_radius - (int)sqrt(corner_radius * corner_radius -
                                               (corner_radius - y) * (corner_radius - y));
        int rect_width = width - 2 * x_start;
        if (rect_width > 0) {
            pixman_region32_union_rect(&clip_region, &clip_region, x_start, y, rect_width, 1);
        }
    }

    /* Middle section */
    pixman_region32_union_rect(&clip_region, &clip_region, 0, corner_radius, width, height - 2 * corner_radius);

    /* Bottom corners */
    for (int y = 0; y < corner_radius; y++) {
        int x_start = corner_radius - (int)sqrt(corner_radius * corner_radius - (y + 1) * (y + 1));
        int rect_width = width - 2 * x_start;
        if (rect_width > 0) {
            pixman_region32_union_rect(&clip_region, &clip_region,
                                       x_start, height - corner_radius + y,
                                       rect_width, 1);
        }
    }

    /* Currently we can’t clip content directly on stable wlroots.
       Keep this function for future extension or for button/corner buffer use. */
